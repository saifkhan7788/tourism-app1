...
const AnnouncementManagement = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL || 'https://tourism-app1-production.up.railway.app/api'}/announcements`, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (editMode) {
        await axios.put(`${process.env.REACT_APP_API_URL || 'https://tourism-app1-production.up.railway.app/api'}/announcements/${formData.id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.post(`${process.env.REACT_APP_API_URL || 'https://tourism-app1-production.up.railway.app/api'}/announcements`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      fetchAnnouncements();
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${process.env.REACT_APP_API_URL || 'https://tourism-app1-production.up.railway.app/api'}/announcements/${selected.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchAnnouncements();

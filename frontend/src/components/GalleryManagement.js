...
const GalleryManagement = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    setUploading(true);
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL || 'https://tourism-app1-production.up.railway.app/api'}/upload/image`, formDataUpload, {
    ...
    <Box>
          ...
          <TableBody>
            {filteredImages.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((image) => (
              <TableRow key={image.id}>
                <TableCell>
                  <img src={`${process.env.REACT_APP_API_URL?.replace('/api', '') || 'https://tourism-app1-production.up.railway.app'}${image.image_url}`} alt={image.title} style={{ width: 100, height: 60, objectFit: 'cover', borderRadius: 4 }} />
                </TableCell>
      ...
      <Dialog open={dialog} onClose={() => setDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          {formData.image_url && (
            <Box sx={{ mb: 2 }}>
              <img src={`${process.env.REACT_APP_API_URL?.replace('/api', '') || 'https://tourism-app1-production.up.railway.app'}${formData.image_url}`} alt="Preview" style={{ maxWidth: '100%', borderRadius: 8 }} />
            </Box>
          )}
